...
def universal_dict_collate_fn(batch, check_padding=True, device=None) -> OrderedDict:
    results = OrderedDict({})
    ref = batch[0]
    for k in ref.keys():
        if isinstance(ref[k], (list, tuple)):
            ...
            results[k] = np.stack(
                ...
                axis=0)
            results[k] = to_tensor(results[k], transpose=False, spatial_dims=0, has_batch=True, device=device)
        else:
            ...
            results[k] = to_tensor(results[k], transpose=True, spatial_dims=2, has_batch=True, device=device)
    return results
