...
# generate distribution depend on give tensor
def generate_dist(x, ):
    assert isinstance(x, torch.tensor)
    print(x.shape) # except [B, N, C]: B is batch size, N is instance number, C is topo states (e.g., r1,r2,area...)
    B, N, C = x.shape

    xmean = torch.mean(x, dim=-1, keepdim=True)
    xstd = torch.std(x, dim=-1, keepdim=True)

    xdis = 1./(xstd*math.sqrt(2*torch.pi)) * torch.exp(-((x-xmean)**2 / (2*xstd**2))) # [B, N, C], in this case, C denote distribution
...
def lookup_nn(item: str, *a, src=None, call=True, inplace=True, nd=None, **kw):
    ...
    """
    if src is None:
        from model import commons, cpn, loss
        from backbone import convnext, ppm, resnet, unet
        from util import logging, schedule, timer, util
        # import model, backbone, util
        # src = (nn, model, backbone, util)
        src = (nn, commons, cpn, loss, convnext, ppm, resnet, unet, logging, schedule, timer, util)
