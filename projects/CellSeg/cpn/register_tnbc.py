...
def register_cell(split='train'):
    ...
    if split == 'train':
        img_files = "/run/user/1000/gvfs/smb-share:server=192.168.2.221,share=fqpathotech/ lingpeng/TNBC_NucleiSegmentation/images_512"
    else:
        img_files = "/run/user/1000/gvfs/smb-share:server=192.168.2.221,share=fqpathotech/ lingpeng/TNBC_NucleiSegmentation/image_test_512"
    all_files = os.listdir(img_files) # ori_imgs
    # if split == 'train':
...
def register_cell_all():
    ...
    if split == 'train':
        ...
        DatasetCatalog.register("tnbc_" + "train", lambda: register_cell())
        MetadataCatalog.get('tnbc_' + "train").set(json_file=None, evaluator_type="refcoco", thing_classes=["cell"])

    elif split == 'test':
        # split = 'test'

        DatasetCatalog.register("tnbc_" + 'test', lambda: register_cell(split=split))
        MetadataCatalog.get('tnbc_' + 'test').set(json_file=None, evaluator_type="refcoco", thing_classes=["cell"])

    elif split == 'cos':
        DatasetCatalog.register("tnbc_" + 'test', lambda: register_cell_cos())
        MetadataCatalog.get('tnbc_' + 'test').set(json_file=None, evaluator_type="refcoco", thing_classes=["cell"])
